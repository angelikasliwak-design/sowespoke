...
- Live-Übersetzung der englischsprachigen RSS-Quellen (Microsoft Advertising Blog, Search Engine Land) ins Deutsche über die Google-Gemini-API (`functions/api/news.js`, Secret `GEMINI_API_KEY`, Modell konfigurierbar über `GEMINI_MODEL`, Default `gemini-2.0-flash`) — läuft serverseitig als Batch-Übersetzung, Ergebnis liegt im ohnehin bestehenden 15-Minuten-Response-Cache. Ohne gesetztes Secret bleibt das bisherige Verhalten (Original-Text + "EN"-Badge) unverändert, kein Fehler. Gemini statt OpenAI gewählt, weil der Nutzer nur ein ChatGPT-Plus-Abo (kein OpenAI-API-Billing) hat, Gemini aber ein kostenloses API-Kontingent bietet.
