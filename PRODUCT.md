...
- **Persistentes News-Archiv** (2026-09-04, Nutzer-Beobachtung: "gestern gab es andere News, z. B. Google Trends, heute sehe ich sie nicht mehr") — RSS-Feeds der Quellen selbst sind kein Archiv, sondern zeigen nur ein rollierendes Fenster der letzten ~10–20 Beiträge; sobald eine Quelle etwas Neues veröffentlicht, fällt der älteste Eintrag aus ihrem eigenen Feed. `functions/api/news.js` merged deshalb jeden live abgerufenen Artikel dauerhaft in einen KV-Speicher (Namespace `NEWS_ARCHIVE`, ein einzelner JSON-Blob unter dem Key `archive:pool`, auf `MAX_ARCHIVE_ITEMS` = 200 Einträge gedeckelt — bewusst EIN Blob statt eines Keys pro Artikel wie sonst in diesem Projekt üblich, siehe Code-Kommentar: verhindert, dass die KV-Lese-Kontingente mit wachsendem Archiv linear mitwachsen). Angezeigt werden weiterhin nur die neuesten 60 (unverändert), jetzt aber aus Live+Archiv gemeinsam statt nur aus dem, was gerade noch live in den Quellen sichtbar ist. **KV-Bindung bestätigt aktiv** (2026-09-07, per Cloudflare-API direkt gegen das Produktions-Deployment geprüft, nicht nur angenommen) — das Archiv läuft. Wichtige Einschränkung: es sammelt nur, was seit Aktivierung der Bindung tatsächlich live abgerufen wurde, kein rückwirkendes Nachladen älterer, bereits aus den Quell-Feeds gefallener Artikel. Stichprobe (2026-09-07): 42 archivierte Artikel seit 2026-04-27, verteilt auf alle vier damaligen Quellen — kein einziger davon erwähnt Meta oder Snapchat (Nutzer-Beobachtung, dass Meta-/Snapchat-Artikel "verschwunden" seien, geprüft: die Kanäle Meta/TikTok/Snapchat/KI/CRO/Rechtliches sind in der Kanal-Leiste zwar als feste Filter-Reiter vorhanden, aber keine der vier Quellen liefert `channel` je diesen Wert — die Kanal-Zuordnung ist pro Quelle fix, nicht inhaltsbasiert. Diese Reiter waren in der echten Produktionsumgebung nie befüllt, nur im lokalen Mock-Datensatz des Vorschau-Servers).
...
- **Login-Log/Nutzungsübersicht** (2026-08-14, Nutzer-Wunsch: sehen wer die Seite nutzt): bei jedem Login (`functions/api/auth/google/callback.js`) wird pro Person ein Eintrag in Cloudflare-KV (`LOGIN_LOG`) aktualisiert (erster Login, letzter Login, Anzahl). Zusätzlich trackt `functions/_middleware.js` echte Nutzung als `lastActiveAt` — höchstens einmal pro Person und Tag (ein Cookie verhindert unnötige KV-Zugriffe bei jedem Request), läuft über `context.waitUntil()` nach dem Zurückgeben der Antwort, damit kein Request langsamer wird. Neue, nur für Admins sichtbare Seite `#/nutzer` (Nav-Link nur für Admins per JS eingeblendet, Server prüft `isAdminEmail` zusätzlich als echte Schranke) zeigt "Zuletzt aktiv" (Tages-Granularität), "Letzter Login" und "Erster Login" pro Person.
...
18 echte Microsoft-/Branchen-Präsentationen liegen als PDF vor (`content/presentations/`), Inhalt wird ausgewertet und nur mit tatsächlich im Dokument enthaltenen Informationen zusammengefasst — keine Ergänzung aus Allgemeinwissen. RSS-Quellen bestätigt funktionsfähig: Microsoft Advertising Blog (offiziell), Search Engine Land, adseed SEA-News, OMR, **OnlineMarketing.de** (2026-09-07 ergänzt, Nutzer-Wunsch nach mehr DACH-relevanten Infos — stündlich aktualisiert, deutschsprachig, breiter als adseed/OMR: deckt auch Plattform-News zu Google/Meta/TikTok/KI ab). t3n wurde am 2026-08-02 wieder entfernt (nur fachfremder Gesamt-Feed verfügbar, kein Marketing-spezifischer Feed). Drei weitere Quellen (THINK with Google, analyticsmania, PPC Hero) sind technisch noch ungeklärt (kein einfacher RSS-Zugang bzw. blockierter Abruf). Wiederkehrende Rubriken ohne Nachrichtenwert werden per Titel-Muster gefiltert (2026-09-07, Nutzer-Fund: Search Engine Lands wöchentliche Job-Angebote-Sammlung) — `EXCLUDED_TITLE_PATTERNS` in `functions/api/news.js`.
