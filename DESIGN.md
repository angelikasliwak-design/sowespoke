...
- **Direktversand (2026-08-10):** neues Feld "E-Mail-Adresse(n) der Kundschaft" direkt unter der Empfänger:in-Auswahl. Aktionsreihe jetzt zweigeteilt: "E-Mail senden" (`.btn--primary`, `<a href="mailto:...">`, öffnet das eigene E-Mail-Programm des Nutzers mit Empfänger/Betreff/Text) und "In Zwischenablage kopieren" (`.btn--secondary`, bisheriges Verhalten, als Fallback bei sehr langem Text/`mailto:`-Längenlimits). Deaktivierter Link-Zustand über `.btn.is-disabled` (`opacity:.5`, `pointer-events:none`, `aria-disabled`) statt nativem `disabled` — `<a>`-Elemente kennen das Attribut nicht.
